...
public class GatewayTwoSidedTargetGWFailOverMultipleOperationsOnTheSameObjectUnderTxnTest extends AbstractGatewayTest {
    ...
    public GatewayTwoSidedTargetGWFailOverMultipleOperationsOnTheSameObjectUnderTxnTest() {
        if (isDevMode()) {
            ...
            synchronized (ids) {
                if (ids.isEmpty() && ids.size() < 5) {
                    ...
                    if (fullTake) {
                        ...
                        for (int i = 0; i < 5; i++)
                            id[i] = ids.get(ids.size() - 1); //get the same object
                    }
                }
            }
            DefaultTransactionDefinition definition = new DefaultTransactionDefinition();
