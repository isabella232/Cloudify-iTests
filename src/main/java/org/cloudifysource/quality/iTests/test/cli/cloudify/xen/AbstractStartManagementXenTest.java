...
public class AbstractStartManagementXenTest extends AbstractXenGSMTest {
	...
	protected void overrideXenServerProperties(XenServerMachineProvisioningConfig machineProvisioningConfig) {
        ...
        AbstractTestSupport.assertTrue("Mapping for " + mac1 + "does not exist",
                mapping.containsKey(mac1));
        AbstractTestSupport.assertTrue("Mapping for " + mac2 + "does not exist",
	...
	public void beforeTest() {
		...
		repetitiveAssertNumberOfGSAsAdded(2, AbstractTestSupport.OPERATION_TIMEOUT);
    ...
    private GridServiceContainer restGscWorkaround() {
		...
		admin.getGridServiceContainers().waitFor(2, AbstractTestSupport.OPERATION_TIMEOUT, TimeUnit.MILLISECONDS);
		...
		if (restGsc == null) {
			AbstractTestSupport.AssertFail("Failed finding rest GSC");
		}
        return restGsc;
    }
	...
	protected void assertManagementStarted() {
		...
		for (GridServiceManager gsm : admin.getGridServiceManagers()) {
            ...
            try {
                ...
                AbstractTestSupport.repetitiveAssertTrue("Failed waiting for REST/WebIU services",
                ...
                AbstractTestSupport.AssertFail("Setup failed", e);
    ...
    private void startAdditionalManagement() {
        ...
        super.startNewVM(0, 0, agentConfig, AbstractTestSupport.OPERATION_TIMEOUT, TimeUnit.MILLISECONDS);
    }
	...
	protected void startAgent(int extraMemoryCapacityInMB, String... zones) {
	    ...
	    super.startNewVM(0, memoryCapacityInMB, agentConfig, AbstractTestSupport.OPERATION_TIMEOUT, TimeUnit.MILLISECONDS);
	}
	...
	private void waitForAgentsShutdown(final List<GridServiceAgent> agentsShutdown) {
		AbstractTestSupport.repetitiveAssertTrue("Failed waiting for all agents to be removed", new RepetitiveConditionProvider() {
            public boolean getCondition() {
                int agentsAlive = 0;
