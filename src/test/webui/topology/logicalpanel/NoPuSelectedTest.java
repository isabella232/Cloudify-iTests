...
public class NoPuSelectedTest extends AbstractSeleniumTest {
	...
	public void startSetUp() {	
		...
		LogUtils.log("deploying processing unit...");
		deployment = new SpaceDeployment("Test1").partitioned(1, 1).maxInstancesPerVM(1).setContextProperty("com.gs.application", "MyApp");
		Pu = gsmA.deploy(deployment.setContextProperty(APPLICATION_CONTEXT_PROPERY, "App"));
		ProcessingUnitUtils.waitForDeploymentStatus(Pu, DeploymentStatus.INTACT);
	}
	...
	public void goTologsTest() throws InterruptedException {
		...
		ProcessingUnit test = admin.getProcessingUnits().getProcessingUnit("Test");
		ProcessingUnit test1 = admin.getProcessingUnits().getProcessingUnit("Test1");
		
		for (ProcessingUnitInstance puInstance : test.getInstances()) {
			assertTrue(logicalPanel.getProcessingUnitInstance(puInstance.getProcessingUnitInstanceName()) != null);
		}
		
		for (ProcessingUnitInstance puInstance : test1.getInstances()) {
			assertTrue(logicalPanel.getProcessingUnitInstance(puInstance.getProcessingUnitInstanceName()) != null);
		}
