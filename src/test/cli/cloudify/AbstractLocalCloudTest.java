...
public class AbstractLocalCloudTest extends AbstractTest {
    ...
    public void afterTest() throws Exception {
        ...
        Set<String> delta = null;
        if (alivePIDs != null) {
            try {
                final Set<String> currentPids = SetupUtils.getLocalProcesses();
                delta = SetupUtils.getClientProcessesIDsDelta(alivePIDs,
                        currentPids);
            } catch (Exception e) {
                LogUtils.log("WARNING Failed to kill get processes ids", e);
            }
            if (delta != null && delta.size() > 0) {
                ...
                try {
                    ...
                    if (testShouldFail) {
                        Assert.fail("There is a leak PIDS [ " + pids + "] are alive");
                    }
                }
            }
