...
public class CustomCommandsOnMultipleInstancesApplicationTest extends AbstractLocalCloudTest {
    ...
    private void checkPrintCommandOnapp() throws IOException, InterruptedException {
        ...
        AbstractTestSupport.assertTrue("Custom command 'print' returned unexpected result from instance #1: " + invokePrintResult
                , invokePrintResult.contains("OK from instance #1"));
        AbstractTestSupport.assertTrue("Custom command 'print' returned unexpected result from instance #2: " + invokePrintResult
    ...
    private void checkContextCommandOnApp() throws IOException, InterruptedException {
        ...
        for (int i = 1; i <= TOTAL_INSTANCES_SERVICE; i++) {
            AbstractTestSupport.assertTrue("Custom command 'runScript' returned unexpected result from instance #" + i + ": " + invokeContextResult
    ...
    private void checkContextCommandOnApp(int instanceId) throws IOException, InterruptedException {
        ...
        AbstractTestSupport.assertTrue("Custom command 'exception' returned unexpected result from instance #" + instanceId + ": " + invokeContextResult
	...
	private void installApplication() throws IOException, InterruptedException, DSLException {
        ...
        final ProcessingUnit pu1 = admin.getProcessingUnits().waitFor(absolutePUNameSimple1, AbstractTestSupport.OPERATION_TIMEOUT , TimeUnit.MILLISECONDS);
        final ProcessingUnit pu2 = admin.getProcessingUnits().waitFor(absolutePUNameSimple2, AbstractTestSupport.OPERATION_TIMEOUT , TimeUnit.MILLISECONDS);
        AbstractTestSupport.assertNotNull(pu1);
        AbstractTestSupport.assertNotNull(pu2);
        AbstractTestSupport.assertTrue("applications was not installed", pu1.waitFor(TOTAL_INSTANCES_SERVICE, AbstractTestSupport.OPERATION_TIMEOUT, TimeUnit.MILLISECONDS));
        AbstractTestSupport.assertTrue("applications was not installed", pu2.waitFor(TOTAL_INSTANCES_SERVICE, AbstractTestSupport.OPERATION_TIMEOUT, TimeUnit.MILLISECONDS));
        AbstractTestSupport.assertNotNull("applications was not installed", admin.getApplications().getApplication(APPLICATION_NAME));
        AbstractTestSupport.assertTrue("USM Service State is NOT RUNNING", USMTestUtils.waitForPuRunningState(absolutePUNameSimple1, 60, TimeUnit.SECONDS, admin));
