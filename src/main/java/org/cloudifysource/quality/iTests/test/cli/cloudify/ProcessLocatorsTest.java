...
public class ProcessLocatorsTest extends AbstractLocalCloudTest {
	...
	private ProcessingUnitInstance findPUI(final ProcessingUnit pu, final String serviceFullName)
		...
		AbstractTestSupport.assertTrue("USM Service state is not RUNNING",
                USMTestUtils.waitForPuRunningState(serviceFullName, 20, TimeUnit.SECONDS, admin));
		AbstractTestSupport.assertTrue("Could not find instance of deployed service", found);
	...
	private ProcessingUnit findPUForService(final String puName) {
		...
		AbstractTestSupport.assertNotNull("Could not find processing unit for installed service",
                pu);
		return pu;
	}
	...
	public void testBackgroundService()
		...
		AbstractTestSupport.assertTrue("port 7777 already in use", !isPortOpen(7777));
		runCommand("connect " + restUrl + ";install-service --verbose " + BG_RECIPE_DIR_PATH);
		AbstractTestSupport.assertTrue("port 7777 not in use, despite service up", isPortOpen(7777));
		final String puName = ServiceUtils.getAbsolutePUName("default", "simple-background");
		AbstractTestSupport.assertEquals("unexpected number of pids", 1, getPIDs(puName).size());

		runCommand("connect " + restUrl + ";uninstall-service --verbose simple-background");
		AbstractTestSupport.assertTrue("port 7777 still in use", !isPortOpen(7777));
	...
	public void testEmptyService()
		...
		AbstractTestSupport.assertEquals("unexpected number of pids", 0, getPIDs(puName).size());
	...
	private List<Long> getPIDs(final String puName)
		...
		if (pidsObject == null) {
			...
			AbstractTestSupport.AssertFail("Unexpected monitors class: " + pidsObject.getClass().getName());
			return null;
		}
	...
	public void testMultiSimpleService()
		...
		AbstractTestSupport.assertTrue("port 7777 already in use", !isPortOpen(7777));
		AbstractTestSupport.assertTrue("port 7778 already in use", !isPortOpen(7778));
		runCommand("connect " + restUrl + ";install-service --verbose " + MULTI_RECIPE_DIR_PATH);
		AbstractTestSupport.assertTrue("port 7777 not in use, despite service up", isPortOpen(7777));
		AbstractTestSupport.assertTrue("port 7778 not in use, despite service up", isPortOpen(7778));

		final String puName = ServiceUtils.getAbsolutePUName("default", "multi-simple");
		AbstractTestSupport.assertEquals("unexpected number of pids", 2, getPIDs(puName).size());

		runCommand("connect " + restUrl + ";uninstall-service --verbose multi-simple");
		AbstractTestSupport.assertTrue("port 7777 still in use", !isPortOpen(7777));
		AbstractTestSupport.assertTrue("port 7778 still in use", !isPortOpen(7778));
