...
# TERM=xterm export TERM
 ...
 #start sgtest
 ${SGTEST_ROOT_BIN_DIR}/sgtest-cmd.sh ${BUILD_NUMBER} ${INCLUDE} ${EXCLUDE} ${SUITE_NAME} ${MAJOR_VERSION} ${MINOR_VERSION} ${SUITE_ID} ${SUITE_NUMBER} ${BYON_MACHINES} ${SUPPORTED_CLOUDS} ${EC2_REGION} ${SUITE_WORK_DIR} ${SUITE_DEPLOY_DIR} ${BRANCH_NAME} ${SUITE_TYPE} ${MAVEN_REPO_LOCAL}
