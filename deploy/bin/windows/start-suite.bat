...
call ant -verbose -buildfile %LOCAL_SGPATH%\bin\run-win.xml testsummary -DBUILD_NUMBER=%BUILD_NUMBER% -DSUITE_NAME=%SUITE_NAME% -DBUILD_DIR=%RUNTIME_BUILD_LOCATION% -DMAJOR_VERSION=%VERSION% -DMINOR_VERSION=%MILESTONE% -DEC2_REGION=%EC2_REGION% -DSELENIUM_BROWSER=%selenium.browser% -DSUITE_ID=%SUITE_ID% -DSUITE_NUMBER=1 -DINCLUDE="%INCLUDE%" -DEXCLUDE="%EXCLUDE%"
