...
public class RemoveSecurityGroupsScript {
	...
	static boolean cleanSecurityGroups() {
		...
		try {
			...
			RestContext<AWSEC2Client, AWSEC2AsyncClient> restContext = buildView.unwrap();
			...
			for (SecurityGroup securityGroup : describeSecurityGroupsInRegion) {
				...
				if (securityGroup.getName().startsWith("jclouds#")) {
                  ...
                  System.out.println("deleting security group " + name);
                  securityGroupServices.deleteSecurityGroupInRegion(region, name);
