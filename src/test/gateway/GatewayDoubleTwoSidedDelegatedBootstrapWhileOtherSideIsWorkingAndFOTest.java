 ...
 * @since 8.0.3
 */
public class GatewayDoubleTwoSidedDelegatedBootstrapWhileOtherSideIsWorkingAndFOTest extends AbstractGatewayTest {
    ...
    public GatewayDoubleTwoSidedDelegatedBootstrapWhileOtherSideIsWorkingAndFOTest() {
        if (isDevMode()) {
    ...
    public void test() throws Exception {
        ...
        final Thread restartThread = new Thread(new Runnable() {
            public void run() {
                restartSitePrimaries(admin3, "NY");
            }
        });
        restartThread.start();
        ...
        while (stopUpdates == false) {
            ...
            if (!fullTake) {
                if (Math.random() < 0.7) {
                    ...
                    remoteGigaSpace.take(new Stock(id), 1000);
                }
            } else {
                Stock stock = remoteGigaSpace.take(new Stock(id), 1000);
                if (stock == null) {
                    synchronized (ids) {
                        ids.add(id);
                    }
