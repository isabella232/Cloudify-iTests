 ...
 * @since 8.0.3
 */
public class GatewayFullGraphSchemaWithDynamicIndexTest extends AbstractGatewayTest {
    ...
    public GatewayFullGraphSchemaWithDynamicIndexTest() {
        if (isDevMode()) {
    ...
    public void test() throws Exception {
        ...
        IndexUtil.addDynamicIndex(gigaSpace1, "info", "com.gatewayPUs.common.MessageGW", SpaceIndexType.BASIC);
        ...
        IndexUtil.addDynamicIndex(gigaSpace2, "info", "com.gatewayPUs.common.MessageGW", SpaceIndexType.BASIC);
        assertCleanSites(spaces, admins);
    ...
    protected void takeMultiple(final GigaSpace source, GigaSpace[] targets, final boolean isProcessed) {
        ...
        for (final GigaSpace target : targets) {
            ...
            TestUtils.repetitive(new Runnable() {
                public void run() {
                    MessageGW[] msgArray = target.readMultiple(new MessageGW(isProcessed), Integer.MAX_VALUE, ReadModifiers.DIRTY_READ);
    ...
    private void initialize() {
        ...
        deploySite(gsmLON, siteDeployment("./apps/gateway/clusterWith2targets", "londonSpace", props)
                .numberOfInstances(2).numberOfBackups(0));
        props.clear();
        ...
        assertGatewayReplicationConnected(space2, 2, 2);
