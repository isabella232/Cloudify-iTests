...
echo creating cassandra schema in 5 seconds
ping localhost -n 1 -w 5000 > nul 2>&1

if not exist createschema.log goto createschema
del /q createschema.log > nul 2>&1

:createschema
call install\bin\cassandra-cli.bat -host localhost -port 9160 -f analytics-cassandra-schema.txt >createschema.log 2>&1
