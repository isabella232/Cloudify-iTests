...
public class MirrorPersistencyAlertTest extends AbstractSeleniumTest {
	...
	public void mirrorPersist() throws InterruptedException {
		...
		log("waiting for resolved alert to appear in the webui");
		final AlertsGrid alertsGrid = dashboardTab.getAlertsGrid();
		RepetitiveConditionProvider condition = new RepetitiveConditionProvider() {
			public boolean getCondition() {
				List<WebUIAlert> webuiAlerts = alertsGrid.getParentAlertsByType(AlertsGrid.MIRROR);
				for (WebUIAlert alert : webuiAlerts) {
					if (alert.getStatus().equals(AlertStatus.RESOLVED)){
						log("found resolved alert : " + alert.toString());
						return true;
					}
				}
				return false;
		...
		takeScreenShot(this.getClass(), "mirrorPersist");
		
		// retrieve mirror alerts from the web-ui
		log("retrieving alerts from webui");
        AlertsGrid alertGrid = dashboardTab.getAlertsGrid();
		List<WebUIAlert> mirrorAlerts = alertGrid.getAlertsByType(AlertsGrid.MIRROR);
		alertGrid.assertAlertsConsistency(mirrorAlerts, adminAlerts);
