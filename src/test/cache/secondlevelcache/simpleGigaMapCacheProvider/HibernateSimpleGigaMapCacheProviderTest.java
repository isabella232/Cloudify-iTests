...
public class HibernateSimpleGigaMapCacheProviderTest extends AbstractTest {
    ...
    public void setup() throws IOException, InterruptedException {
        ...
        hsqldbProcessId = DBUtils.loadHSQLDB(machine, "SimpleGigaMapCacheProvider", dbPort);

        String location = "/test/cache/secondlevelcache/resources/simple-gigamap-hibernate-applicationcontext.xml";
        ApplicationContext ctx = new ClassPathXmlApplicationContext(location);
        sessionFactory = (SessionFactory) ctx.getBean("sessionFactory");
    }

    @Test(timeOut = DEFAULT_TEST_TIMEOUT, groups = "1")
    public void testSimpleGigaMapCacheProvider() throws Exception {
        ...
        UrlSpaceConfigurer configurer = new UrlSpaceConfigurer("jini://*/*/SimpleGigaMapCacheProviderSpace");
        configurer.lookupGroups(admin.getGroups()[0]);
        GigaSpace gigaSpace = new GigaSpaceConfigurer(configurer.space()).gigaSpace();

        Assert.assertEquals(OBJECTS+1, gigaSpace.count(new Envelope()));
        ...
        for (int i = 0; i < OBJECTS; i++) {
            session.saveOrUpdate(new Person(Long.valueOf(i), "updated twice name" + i));
        }
        txn4.commit();
        session.close();
        ...
        Transaction txn5 = session1.beginTransaction();
        session1.clear();
        session1.evict(Person.class);
        session1.flush();
        txn5.commit();
