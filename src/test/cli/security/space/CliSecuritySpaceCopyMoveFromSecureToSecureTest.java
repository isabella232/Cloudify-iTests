 ...
 *
 */
public class CliSecuritySpaceCopyMoveFromSecureToSecureTest extends CliSecurityAbstractSpaceTest {
    ...
    public void testSpaceCopyMoveWithGoodCredentials() {
        ...
        // Assert 'space move'
        int aGigaSpaceCount = aGigaSpace.count(null);
        for (int i = 0; i < 5; i++) {
            if (aGigaSpaceCount == 0) {
                break;
            }
            try {
                Thread.sleep(1000);
            ...
            aGigaSpaceCount = aGigaSpace.count(null);
        }
        
        assertEquals(0, aGigaSpaceCount);
    ...
    public void testSpaceCopyMoveWithInsufficientPrivileges() {
        ...
        regex = "User \\[Reader\\] lacks \\[Write\\] privileges";
        assertEquals("Testing: " + regex, 1, CliUtils.patternCounter(regex, mainOutput));
