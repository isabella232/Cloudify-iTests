 ...
 * 
 * TEST (2) Machine GSC Failure and recovery
 * 
 ...
 * c. Kill -9 GSC A1, GSC A2
 * 	  Space instances from A1, A2 are not re-deployed because of max-per-machine
 *    2 Spaces are available both primary
 * d. Restart GSC A1, GSC A2
 *    2 backups are started on A1 and/or A2
 * e. 2 clusters are available, 4 spaces total
 * 
 */
public class MachineGscFailureAndRecoveryTest extends AbstractTest {
	...
	//Kill -9 GSC A1, GSC A2
	//Space instances from A1, A2 are not re-deployed because of max-per-machine
	//2 Spaces are available both primary
	private void killGSCsOnMachineA() {
		...
		log("killing GSC A2, containing: " + ProcessingUnitUtils.getProcessingUnitInstanceName(gscA2.getProcessingUnitInstances()));
		gscA1.kill();
		gscA2.kill();
	...
	//Restart GSC A1, GSC A2
	//2 backups are started on A1 and/or A2
	private void restartGSCsOnMachineA() throws Exception {
		log("restart GSC A1, GSC A2");
		GridServiceContainer[] gscs = AdminUtils.loadGSCs(machineA, 2);
