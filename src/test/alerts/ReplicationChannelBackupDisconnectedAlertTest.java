...
public class ReplicationChannelBackupDisconnectedAlertTest extends AbstractTest {
	...
	public void replicationChannelBackupDisconnected() throws InterruptedException {
		...
		for (int i = 0 ; i < processingUnits.length ; i++) {
			...
			if (puInst.getSpaceInstance().getMode().equals(SpaceMode.BACKUP)) {
				LogUtils.log("Killing backup space instance: " + puInst.getSpaceInstance());
		...
		Assert.assertNotNull(((ReplicationChannelDisconnectedAlert)raisedAlert).getSourceVirtualMachineUid());
		Assert.assertNotNull(((ReplicationChannelDisconnectedAlert)resolvedAlert).getSourceVirtualMachineUid());
	}
}
