 ...
 * Tests:
 * - 4.11.2.1: Deploy servlet-test-remote (plain mode) and deploy space named mySpace
 * with 2 instances sync_replicated
 * Verify that servlet is accessible by http://IP:8080/session-test
 * Verify that is "New Session" is has "true" value.
 * 
 * - 4.11.2.2 : Add new attribute name and value (new-name:new-value).
 * Verify that the key-value pair was put in the session object in the embedded space 
 * 
 * - 4.11.2.3: Undeploy the webapp.
 * 
 */
public class DistributedHttpSessionRemoteTest extends AbstractWebTest {
    ...
    public void test() throws Exception {
        ...
        ProcessingUnit mySpace = gsm.deploy(
                new SpaceDeployment("mySpace").
                numberOfInstances(2).clusterSchema("sync_replicated"));
        
        mySpace.waitFor(mySpace.getTotalNumberOfInstances());
        
        ProcessingUnit pu = gsm.deploy(
                new ProcessingUnitDeployment(getArchive("session-test-remote.war")).
                numberOfInstances(2).
        ...
        SessionData sessionData = mySpace.getSpace().getGigaSpace().read(null, 60000);
