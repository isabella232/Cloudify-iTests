 ...
 * @since 8.0.3
 */
public class GatewayDoubleTwoSidedWithRoutingSchemaTest extends AbstractGatewayTest {
	...
	public GatewayDoubleTwoSidedWithRoutingSchemaTest() {
		if (isDevMode()) {
	...
	public void test() throws Exception {
		...
		log("case #" + ++caseNum + " write");
		write(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " writeMultiple");
		writeMultiple(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " update");
		update(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " updateMultiple");
		updateMultiple(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " take");
		take(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " takeMultiple");
		takeMultiple(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		targets[0] = gigaSpace2;

		log("case #" + ++caseNum + " writeNotExpectingReplication");
		writeNotExpectingReplication(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " writeMultipleNotExpectingReplication");
		writeMultipleNotExpectingReplication(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " updateNotExpectingReplication");
		updateNotExpectingReplication(gigaSpace3, targets, false);
		assertCleanSites(spaces, admins);

		log("case #" + ++caseNum + " updateMultipleNotExpectingReplication");
	...
	private void initialize() {
		...
		deploySite(gsm3, siteDeployment("./apps/gateway/cluster", "nySpace", props));
		props.clear();
		...
		deployGateway(gsm2, siteDeployment("./apps/gateway/gateway-components", "LONDON-1", props));
		props.clear();

		props.put("localGatewayName", "LONDON");
		props.put("targetGatewayName", "NY");
		props.put("targetDelegateThrough", "ISRAEL");
		props.put("localClusterUrl", "jini://*/*/londonSpace?groups=" + group2);
		props.put("gatewayLookupGroup", UUID.randomUUID().toString());
		props.put("localGatewayHost", host2);
		props.put("localGatewayDiscoveryPort", "10002");
		...
		props.put("targetGatewayDiscoveryPort", "10001");
		props.put("targetGatewayLrmiPort", "7000");
		deployGateway(gsm2, siteDeployment("./apps/gateway/gatewayRoutingComponents", "LONDON-2", props));
		props.clear();
		...
		props.put("target2GatewayLrmiPort", "7003");
		deployGateway(gsm3, siteDeployment("./apps/gateway/gatewayTwoSourcesWithDelegator", "NY", props));
		props.clear();
		...
		assertGatewayReplicationConnected(space1, 2);
		assertGatewayReplicationConnected(space2, 2);
		assertGatewayReplicationConnected(space3, 1);
