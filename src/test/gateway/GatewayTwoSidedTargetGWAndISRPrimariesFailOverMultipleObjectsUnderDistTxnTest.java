...
public class GatewayTwoSidedTargetGWAndISRPrimariesFailOverMultipleObjectsUnderDistTxnTest extends AbstractGatewayTest {
    ...
    public GatewayTwoSidedTargetGWAndISRPrimariesFailOverMultipleObjectsUnderDistTxnTest() {
        if (isDevMode()) {
    ...
    public void test() throws Exception {
        ...
        Assert.assertNotNull(gigaSpace1.read(new Stock()));
        Assert.assertNotNull(gigaSpace2.read(new Stock()));
        TestUtils.repetitive(new Runnable() {
			@Override
			public void run() {
				AssertUtils.assertEquals(gigaSpace1.count(new Stock()), gigaSpace2.count(new Stock()));
			}
    ...
    private List<Integer> fillSpace(int numberOfIds, GigaSpace remoteGigaSpace, PlatformTransactionManager ptm) {
        ...
        for (int id = 0; id < numberOfIds / 1000; id++) {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                {
                    td.setId(i + (id * 1000));
                    td.setRouting(Integer.valueOf((int) (Math.random() * 100000)));
    ...
    private void deployIsraelSite(GridServiceManager gsm1) {
        ...
        deploySite(gsm1, siteDeployment("./apps/gateway/cluster", "israelSpace", props).numberOfInstances(2).numberOfBackups(1));
    }
    ...
    public static class Stock {
        ...
        public boolean equals(Object o) {
            ...
            if (routing != null ? !routing.equals(stock.routing) : stock.routing != null) return false;
