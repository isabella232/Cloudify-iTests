...
/export/utils/ant/apache-ant-1.8.1/bin/ant -d -DSUITE_TYPE=${SUITE_TYPE} -DBUILD_NUMBER=${BUILD_NUMBER} -DSUITE_NAME=${SUITE_NAME} -DMAJOR_VERSION=${MAJOR_VERSION} -DMINOR_VERSION=${MINOR_VERSION} -f ${DEPLOY_ROOT_BIN_DIR}/../../bin/post-run.xml report-merger
