 ...
 *
 */
public class HoldAndReleaseUIDDistributedTxnTest extends AbstractGatewayTest {
	...
	private final String FILTER_WAITING_TIME = "4000";
	...
	public HoldAndReleaseUIDDistributedTxnTest() {
		if (isDevMode()) {
	...
	private void case1() throws Exception {
		...
		TransactionStatus status = ptmLondon.getTransaction(definition);
		txnGigaSpaceLondon.write(new Stock(0));
		ptmLondon.commit(status);
		...
		RepetitiveConditionProvider repCond = new RepetitiveConditionProvider() {
			...
			public boolean getCondition() {
				...
				return txnGigaSpaceLondon.count(new Stock()) == 1;
			}
		};

		
		
		log("assert there is one object in london for " + (Integer.valueOf(TXN_TO)/2));
		AssertUtils.repetitiveAssertConditionHolds("there is no one object in the remote space", repCond, (Integer.valueOf(TXN_TO)/2), 200);
		...
		repCond = new RepetitiveConditionProvider() {
			...
			public boolean getCondition() {
				...
				MessageGW template = new MessageGW(1);
				template.setInfo("txn");
				return txnGigaSpaceLondon.count(new Stock()) == 2 && readStock.getVersion() == 2
					&& txnGigaSpaceLondon.count(template) == 1;
			}
		};
		log("assert there no obejects in london for " + (int)DEFAULT_TEST_TIMEOUT);
		repetitiveAssertTrue("object amount in the remote space is not 1", repCond, (int)DEFAULT_TEST_TIMEOUT);
