...
export UPLOAD_NAME="upload_STORAGE_TEMPLATE"
echo UPLOAD_NAME value, should be upload_STORAGE_TEMPLATE, -- $UPLOAD_NAME 
...
if [ "$GSA_MODE" = "agent" ]; then
	ERRMSG="${ERRMSG} agent"
	START_COMMAND_ARGS="${START_COMMAND_ARGS}agent -timeout 30 --verbose"
