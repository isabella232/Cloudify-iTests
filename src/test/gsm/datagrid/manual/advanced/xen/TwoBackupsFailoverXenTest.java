 ...
 * @author giladh
 */
public class TwoBackupsFailoverXenTest extends AbstractXenGSMTest {
    ...
    public void doTest() {
        ...
        final ProcessingUnit pu = gsm.deploy(
                new ElasticSpaceDeployment("twobackups_space")
                .numberOfBackupsPerPartition(2)                
        		.maxMemoryCapacity(NUM_CONTAINERS*MEM_PER_CONTAINER, MemoryUnit.MEGABYTES)
                .maxNumberOfCpuCores(NUM_MACHINES*2)
        ...
        assertEquals("Number of pu backups", pu.getNumberOfBackups(), 2);
		...
		GsmTestUtils.writeData(pu, NUM_OBJECTS);
    	...
    	int afterCount = GsmTestUtils.countData(pu);
    	
    	assertEquals("Make sure no objects were lost upon killing of 2 machines", 
    			      afterCount, NUM_OBJECTS);
