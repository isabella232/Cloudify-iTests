	...
	protected void startManagement(String machine1) throws Exception {
		...
		for (int i = 0 ; i < 3 ; i++) {
			try {
				...
				SSHUtils.runCommand(machine1, AbstractTestSupport.DEFAULT_TEST_TIMEOUT,  ByonCloudService.BYON_HOME_FOLDER + "/gigaspaces/tools/cli/cloudify.sh start-management --verbose -cloud-file "  + cloudFile, ByonCloudService.BYON_CLOUD_USER, ByonCloudService.BYON_CLOUD_PASSWORD);
				return;
			} catch (Throwable t) {
