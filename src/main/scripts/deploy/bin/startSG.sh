...
for ((id=0 ; id < ${SUITE_NUMBER} ; id++ )); do
 ...
 ${PDSH} -w ssh:pc-lab[${TARGET_CLIENT_MACHINE}] "${CLIENT_EXECUTOR_SCRIPT} ${DEPLOY_ROOT_BIN_DIR}
 ${SGTEST_CLIENT_BUILD_DIR} ${CONFIG_JAVA_ORDER} ${LOOKUPGROUPS} ${BUILD_NUMBER} ${INCLUDE} ${EXCLUDE} ${SUITE_NAME}
 ${MAJOR_VERSION} ${MINOR_VERSION} ${SUITE_ID} ${SUITE_NUMBER} ${BYON_MACHINES} ${SUPPORTED_CLOUDS} ${EC2_REGION}
 ${SUITE_WORK_DIR} ${SUITE_DEPLOY_DIR} ${BRANCH_NAME} ${SUITE_TYPE} ${MAVEN_REPO_LOCAL} ${MAVEN_PROJECTS_VERSION_XAP} ${MAVEN_PROJECTS_VERSION_CLOUDIFY} ${ENABLE_LOGSTASH} ${S3_MIRROR} ${BACKWARDS_RECIPES_BRANCH}" &
