 ...
 *
 */
public class RedoLogDistributedTxnTest extends AbstractGatewayTest {
	...
	public RedoLogDistributedTxnTest() {
		if (isDevMode()) {
	...
	public void test() throws Exception {
		...
		// Make sure objects replicated to london so next operations
		// will be in a new batch
		TestUtils.repetitive(new Runnable() {
            @Override
            public void run() {
                Stock template = new Stock();
                int count = txnGigaSpaceLondon.count(template);
                Assert.assertEquals(8, count);
            }
		}, OPERATION_TIMEOUT);
		
		// Write additional two stocks for clearing pending packets from process log (redo-log)
		Stock[] stocks = new Stock[2];
		stocks[0] = new Stock(10000);
		stocks[1] = new Stock(10001);
		txnGigaSpaceIsrael.writeMultiple(stocks);
