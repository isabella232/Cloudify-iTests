...
<xsl:template name="package">
  ...
  <xsl:for-each select="/testsuites/testsuite[@package = $name]">
    ...
    <xsl:if test="string-length(./dump)!=0">
      <redirect:write file="{$output.dir}/{$package.dir}/{@id}_{@name}-out.txt">
        <xsl:value-of disable-output-escaping="yes" select="./dump"/>
      </redirect:write>
    </xsl:if>
    <xsl:if test="string-length(./test-log)!=0">
      <redirect:write file="{$output.dir}/{$package.dir}/{@id}_{@name}-err.txt">
        <xsl:value-of disable-output-escaping="yes" select="./test-log"/>
      </redirect:write>
    </xsl:if>
