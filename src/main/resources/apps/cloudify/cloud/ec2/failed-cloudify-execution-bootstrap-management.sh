...
nohup ./wrongpath/cloudify.sh $START_COMMAND $START_COMMAND_ARGS

RETVAL=$?
echo cat nohup.out
cat nohup.out
echo "After cat nohup.out, RETVAL is: $RETVAL"
if [ $RETVAL -ne 0 ]; then
	RETVAL=255
	echo "Calling error_exit with RETVAL: $RETVAL and ERRMSG: $ERRMSG"
