...
public class StartDetectionTest extends AbstractLocalCloudTest {
	...
	public void startDetectionUsingExitCodeTest() throws IOException, InterruptedException {
		...
		AbstractTestSupport.assertTrue("Expected processing unit instance number should be 1", simplePu.waitFor(1, Constants.PROCESSINGUNIT_TIMEOUT_SEC, TimeUnit.SECONDS));
		...
		AbstractTestSupport.assertTrue("service processing unit does not exist", admin.getProcessingUnits().getProcessingUnit(absolutePUName) != null);
		AbstractTestSupport.assertTrue("service should not be installed.", !admin.getProcessingUnits().getProcessingUnit(absolutePUName).getStatus().equals(DeploymentStatus.INTACT));
		simplePu = admin.getProcessingUnits().waitFor(absolutePUName, Constants.PROCESSINGUNIT_TIMEOUT_SEC, TimeUnit.SECONDS);
		AbstractTestSupport.assertTrue("Expected processing unit instances of the service to be 1", simplePu.waitFor(1, Constants.PROCESSINGUNIT_TIMEOUT_SEC, TimeUnit.SECONDS));
		AbstractTestSupport.assertTrue("Service " + absolutePUName + " State is RUNNING.",
